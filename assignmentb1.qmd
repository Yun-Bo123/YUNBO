...
```{r exercise2.2}
b=matrix(rep(c(10,-10,10),times=10),nrow=15,ncol = 3,byrow = TRUE)
print(b)
value=crossprod(b)
print(value)
```
...
print(Y)
x = which(upper.tri(Y) & Y > 75, arr.ind = TRUE)
print(x)
```
...
tmpFn = function(xVec) {
  ...
  s1 = sum((xVec[2:n] - xbar) * (xVec[1:(n-1)] - xbar))
  r1 = s1 / s0
  
  if (n > 2) {

    s2 = sum((xVec[3:n] - xbar) * (xVec[1:(n-2)] - xbar))
    r2 = s2 / s0
  } else {
    r2 = NA
  }
...
myList <- lapply(0:999, function(i) sapply(1:10, function(j) j + i*10))
yList <- lapply(0:999, function(i) sapply(1:10, function(j) j*2 + i*5))
...
bottom <- 0
for (i in 1:1000) {
  top <- top + i * myList[[i]][2]
  bottom <- bottom + i * yList[[i]][2]
}
c <- top / bottom
print(c)
```
